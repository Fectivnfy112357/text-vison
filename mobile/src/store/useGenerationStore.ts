    ...
    (set, get) => ({
      ...
      restorePolling: () => {
        ...
        if (currentGeneration && currentGeneration.status === 'processing') {
          get().startPolling(currentGeneration.id)
        }
      },

      // 手动刷新当前生成状态
      refreshCurrentGeneration: async () => {
        const { currentGeneration } = get()
        if (currentGeneration && currentGeneration.status === 'processing') {
          await get().checkGenerationStatus(currentGeneration.id)
        }
      },
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        // 当存储恢复时，如果有处理中的任务，重新开始轮询
        if (state?.currentGeneration?.status === 'processing') {
          setTimeout(() => {
            state.restorePolling()
          }, 1000) // 延迟1秒开始轮询，确保组件已挂载
        }
      },
    }
  )
